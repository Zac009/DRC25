...
class Vision:
    ...
    def adaptive_centerline(self, mask_yellow, mask_blue, num_steps=1, step_size=10):
        ...
        for _ in range(num_steps):
            ...
            cv2.line(self.final_final_mask, left_pt, right_pt, 255, 1)
            ...
            if yellow_coords is not None and blue_coords is not None:
                ...
                cv2.circle(self.final_final_mask, midpoint, 3, (255, 255, 255), -1)
    ...
    def main(self):
        ...
        while True:
            ...
            blue_mask2 = np.zeros_like(blue_mask)
            ...
            if lines is not None:
                for line in lines:
                    x1, y1, x2, y2 = line[0]
                    cv2.line(blue_mask2, (x1, y1), (x2, y2), 255, 2)  # green lines
            ...
            for cnt in contours:
                ...
                if area > 250:  # adjust threshold as needed
                    cv2.drawContours(blue_mask2, [cnt], -1, 255, 2)

            mask = cv2.morphologyEx(yellow_mask, cv2.MORPH_CLOSE, kernel)
            yellow_mask2 = np.zeros_like(blue_mask)
            ...
            if lines is not None:
                for line in lines:
                    x1, y1, x2, y2 = line[0]
                    cv2.line(yellow_mask2, (x1, y1), (x2, y2), 255, 2)  # green lines
            ...
            for cnt in contours:
                area = cv2.contourArea(cnt)
                if area > 250:  # adjust threshold as needed
                    cv2.drawContours(yellow_mask2, [cnt], -1, 255, 2)

            self.final_final_mask = cv2.add(yellow_mask2, blue_mask2)
            path_points, mask3, ang = self.adaptive_centerline(yellow_mask2, blue_mask2)
            ...
            cv2.imshow('frame3', self.final_final_mask)
